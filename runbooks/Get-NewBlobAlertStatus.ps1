...
while ($null -eq (($alert = Get-AzAlert | Where-Object {$_.Name -like "New Blob Uploaded to AVD*" -and $_.State -eq "New"} | Sort-Object -Property StartDateTime | Select-Object -Last 1 ))) {
...
try
{
    ...
    while ((($alert = Get-AzAlert | Where-Object {$_.Name -like "New Blob Uploaded to AVD*"} `
    ...
    $alert = (Get-AzAlert | Where-Object {($_.Name -like "New Blob Uploaded to AVD*") -and ($_.State -eq "Closed")} `
